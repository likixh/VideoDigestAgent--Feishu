...
def send_summary_email(
    ...
    if platform == "bilibili":
        ...
        thumbnail_url = ""  # Bilibili thumbnails require auth; omit for now
    else:
    ...
    platform_label = f"[{platform.title()}] " if platform != "youtube" else ""
    channel_display = f"@{channel}" if platform == "youtube" else channel

    msg = EmailMessage(policy=SMTP_POLICY)
    msg["Subject"] = f"{platform_label}[{channel_display}] New Video Summary: {video_title}"
    msg["From"] = config.SENDER_EMAIL
    msg["To"] = ", ".join(recipients)

    # Build plain text body
    text_parts = [
        f"New video from {channel_display}\n"
    ...
    # Thumbnail block (only for YouTube which has public thumbnail URLs)
    thumbnail_html = ""
    if thumbnail_url:
        thumbnail_html = f"""\
    ...
    # Platform badge for non-YouTube platforms
    platform_badge = ""
    if platform != "youtube":
        platform_colors = {"bilibili": "#00a1d6"}
        bg_color = platform_colors.get(platform, "#666")
        platform_badge = (
            f'<span style="background: {bg_color}; color: white; padding: 3px 10px; '
            f'border-radius: 12px; font-size: 12px; font-weight: bold; margin-right: 6px;">'
    ...
    <details>
      <summary style="cursor:pointer;font-weight:bold;color:#555;font-size:14px;padding:6px 0;">
        Original Transcript
      </summary>
      <div style="margin-top:10px;padding:12px;background:#f9f9f9;border-radius:6px;
...
<body style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
  <div style="background: #1a1a2e; color: white; padding: 20px; border-radius: 8px 8px 0 0;">
    ...
    <p style="margin: 8px 0 0 0; color: #ccc;">from {channel_display}</p>
  </div>
  <div style="border: 1px solid #ddd; border-top: none; padding: 20px; border-radius: 0 0 8px 8px;">
    ...
    <div style="margin-bottom: 12px;">
      {platform_badge}<span style="background: #e94560; color: white; padding: 3px 10px; border-radius: 12px;
    ...
    <p style="color: #888; font-size: 12px;">
      Generated by Video Summarizer Agent | LLM: {config.LLM_PROVIDER}
    </p>
  </div>
</body>
</html>"""
